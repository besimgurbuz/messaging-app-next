...
route.post('/', registerValidator, async (req, res) => {
  ...
  try {
    ...
    // if new user saved successfully result will be the saved object, or not it will be undefined
    ...
    res.status(400);
    return res.json(`User cannot be saved - ${JSON.stringify(crediatials)}`);
  } catch (err) {
    res.status(400);
    return res.json(err);
  }
